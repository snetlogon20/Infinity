...
from dataIntegrator.modelService.financialAnalysis.SORAnalysisTest import SORAnalysisTest
from dataIntegrator.modelService.financialAnalysis.SORAnalysisReport import SORAnalysisReport

logger = CommonLib.logger


class RunSORAnalysisReport:
    """SOR 分析报告生成器"""

    def __init__(self):
        self.sorAnalysisTest = SORAnalysisTest()
        self.sorAnalysisReport = SORAnalysisReport()
    ...
    def generate_report(self, stock_type="cn_blue_chip", start_date=None, end_date=None,
        ...
        results = self.sorAnalysisTest.run_sor_analysis(
    ...
    def run_batch_reports(self, report_configs):
        ...
        for idx, config in enumerate(report_configs, 1):
            logger.info("\n" + "=" * 80)
            logger.info(f" 开始生成第 {idx}/{len(report_configs)} 个 SOR 分析报告")
            ...
            self.job_logger.start_job('SORAnalysisReport', 'SORAnalysis',
        ...
        if all_results:
            logger.info("\n" + "=" * 80)
            logger.info(" 开始生成 SOR 综合分析研究报告")
            logger.info("=" * 80)

            comprehensive_pdf_path = self.sorAnalysisReport.generate_comprehensive_pdf(
                all_results=all_results,
                report_title="SOR (索提诺比率) 综合分析研究报告"
            )
        ...
        logger.info(f" 报告输出目录: {self.sorAnalysisReport.output_dir}")
        logger.info("=" * 80)
...
if __name__ == "__main__":
    ...
    使用示例 - 批量生成 SOR 分析报告
    """
    runReport = RunSORAnalysisReport()
