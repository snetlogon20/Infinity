...
本文件用于批量生成 SML（证券市场线）分析报告，支持多种股票组合类型
"""
...
class RunSMLAnalysisReport:
    ...
    def generate_report(self, stock_type="cn_blue_chip", start_date=None, end_date=None,
        ...
        results = self.smlAnalysisTest.run_sml_analysis(
    ...
    def run_batch_reports(self, report_configs):
        ...
        for idx, config in enumerate(report_configs, 1):
            ...
            try:
                ...
                logger.info(f"   β值数量: {len(result['betas'])}")
        ...
        if all_results:
            ...
            comprehensive_pdf_path = self.smlAnalysisReport.generate_comprehensive_pdf(
        ...
        logger.info(f"📁 报告输出目录: {self.smlAnalysisReport.output_dir}")
        logger.info("=" * 80)
...
if __name__ == "__main__":
    ...
    使用示例 - 批量生成 SML 分析报告
    """
    runReport = RunSMLAnalysisReport()
