...
本文件用于批量生成信息比率 (Information Ratio) 分析报告，支持多种股票组合类型
"""
...
from dataIntegrator.modelService.financialAnalysis.InformationRatioAnalysisTest import InformationRatioAnalysisTest
...
class RunInformationRatioAnalysisReport:
    """信息比率分析报告生成器"""

    def __init__(self):
        self.irAnalysisTest = InformationRatioAnalysisTest()
        self.irAnalysisReport = InformationRatioAnalysisReport()
    ...
    def generate_report(self, stock_type="cn_blue_chip", start_date=None, end_date=None,
        ...
        results = self.irAnalysisTest.run_ir_analysis(
        ...
        return {
            'name': case_name,
            'market_type': results.get('market_type', 'CN'),
    ...
    def run_batch_reports(self, report_configs):
        ...
        for idx, config in enumerate(report_configs, 1):
            logger.info("\n" + "=" * 80)
            logger.info(f"📊 开始生成第 {idx}/{len(report_configs)} 个信息比率分析报告")
            ...
            self.job_logger.start_job('InformationRatioAnalysisReport', 'InformationRatio',
            ...
            try:
                ...
                self.job_logger.end_job_success(records_processed=len(result.get('information_ratios', [])))
        ...
        if all_results:
            logger.info("\n" + "=" * 80)
            logger.info("📝 开始生成信息比率综合分析研究报告")
            logger.info("=" * 80)

            comprehensive_pdf_path = self.irAnalysisReport.generate_comprehensive_pdf(
                all_results=all_results,
                report_title="信息比率 (IR) 综合分析研究报告"
            )
        ...
        logger.info(f"📁 报告输出目录: {self.irAnalysisReport.output_dir}")
        logger.info("=" * 80)
...
if __name__ == "__main__":
    ...
    使用示例 - 批量生成信息比率分析报告
    """
    runReport = RunInformationRatioAnalysisReport()
