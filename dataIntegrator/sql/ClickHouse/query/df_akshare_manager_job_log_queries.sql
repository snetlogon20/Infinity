-- =====================================================
-- AkShare Task Execution Log Common Query SQL
-- =====================================================

SELECT today()

SELECT * FROM indexsysdb.df_akshare_manager_job_log

DESCRIBE TABLE indexsysdb.df_akshare_manager_job_log;
...
SELECT
    ...
    round(count() / (SELECT count()
                     FROM indexsysdb.df_akshare_manager_job_log
                     WHERE start_time >= toDateTime('2026-05-04 00:00:00', 'Asia/Shanghai')
                     AND start_time < toDateTime('2026-05-05 00:00:00', 'Asia/Shanghai')
                    ) * 100, 2) AS percentage
FROM indexsysdb.df_akshare_manager_job_log
WHERE
