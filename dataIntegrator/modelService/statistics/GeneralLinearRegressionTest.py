...
(
	...
	SELECT  -- 此条就是你要测试的数据，非常妙的办法
	    '20300426' AS df_sys_calendar__trade_date,
...
def load_param_dict_gold_close_analysis_005():
    """
    分析上海金收盘价与各影响因素的关系（基于收盘价而非涨跌幅）
    """
    response_dict = {}
...
ORDER BY df_sys_calendar__trade_date
    ...
    df_tushare_usd_index_daily__USDX_index,
    df_tushare_usd_index_daily__USDCNH_ask_close,
    df_akshare_futures_foreign_hist__CL_close,
    df_akshare_futures_foreign_hist__OIL_close,
    df_akshare_futures_foreign_hist__NG_close
    """
    yColumn = "df_akshare_spot_hist_sge__close"
...
if __name__ == "__main__":
    ...
    '''
        预测上海金收盘价，和股票、纽约金、伦敦金收盘价、美元指数数据字段的关系
    '''
    ...
    '''
        预测上海金收盘价，和股票、纽约金、伦敦金收盘价、美元指数、石油数据字段的关系
    '''
    response_dict = load_param_dict_gold_close_analysis_005()
    generalLinearRegression = GeneralLinearRegression()
    response_dict = generalLinearRegression.run_linear_regression_by_AI(response_dict)
    full_test_df = response_dict["full_test_df"]
    dataframe_corr_df = response_dict["dataframe_corr_df"]

    fileName = FileUtility.generate_filename_by_timestamp(rf"prediction_with_sample_data", "xlsx")
    db_persistent_path = FileUtility.get_full_outbound_path("outbound",fileName)
    full_test_df.to_excel(db_persistent_path)

    fileName = FileUtility.generate_filename_by_timestamp(rf"corr_with_sample_data", "xlsx")
    db_persistent_path = FileUtility.get_full_outbound_path("outbound",fileName)
    dataframe_corr_df.to_excel(db_persistent_path)