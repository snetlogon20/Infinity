...
def test3_calcuate_lda():
    ...
    # 根据图片内容定义输入数据
    frequency_distribution = {
        'probability': np.array([0.5,0.3,0.15,0.05]),  # 概率
        'frequency': np.array([0, 1, 2, 3])  # 频率
    }
    severity_distribution = {
        'probability': np.array([0.6, 0.3, 0.07, 0.03]),  # 概率
        'severity': np.array([1000, 10000, 100000, 1000000])  # 严重程度（美元）
    }
    confidence_level = 0.95  # 95% 置信水平
    print("=" * 60)
    print("操作风险分析程序")
