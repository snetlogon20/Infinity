"""
信息比率分析测试 - 支持股票 + 商品混合分析

本文件演示如何使用 InformationRatioAnalysis 进行多资产类别的投资组合分析
"""
...
from dataIntegrator.modelService.financialAnalysis.InformationRatioAnalysis import InformationRatioAnalysis
from dataIntegrator.modelService.financialAnalysis.InformationRatioAnalysisReport import InformationRatioAnalysisReport

logger = CommonLib.logger


class InformationRatioWithCommoditiesTest:
    """信息比率分析测试类 - 支持商品数据"""

    def run_ir_with_commodities(self, stock_type="cn_blue_chip", start_date="20250101",
                                end_date=None, risk_free_rate=None, interest_country=None,
        ...
        logger.info("=" * 80)
        logger.info("🎯 开始信息比率 (Information Ratio) 分析（含商品）")
        ...
        # 执行信息比率分析
        irAnalysis = InformationRatioAnalysis()
        results = irAnalysis.generate_ir_report(
            stocks=stocks,
...
if __name__ == "__main__":
    """
    使用示例 - 批量生成带商品的信息比率分析报告
    """
    irTest = InformationRatioWithCommoditiesTest()
    irAnalysisReport = InformationRatioAnalysisReport()
    ...
    for idx, config in enumerate(report_configs, 1):
        logger.info("\n" + "=" * 80)
        logger.info(f"📊 开始生成第 {idx}/{len(report_configs)} 个信息比率分析报告（含商品）")
        ...
        try:
            ...
            results = irTest.run_ir_with_commodities(
            ...
            all_results.append({
                ...
                'information_ratios': results['information_ratios'],
                'active_returns': results['active_returns'],
    ...
    if all_results:
        logger.info("\n" + "=" * 80)
        logger.info("📄 开始生成信息比率综合分析研究报告（含商品资产）")
        logger.info("=" * 80)

        comprehensive_pdf_path = irAnalysisReport.generate_comprehensive_pdf(
            all_results=all_results,
            report_title="信息比率 (IR) 综合分析研究报告（含商品资产）"
        )
    ...
    logger.info("\n" + "=" * 80)
    logger.info("🎉 所有信息比率分析任务完成！")
    logger.info(f"📁 报告输出目录: {irAnalysisReport.output_dir}")
    logger.info("=" * 80)
