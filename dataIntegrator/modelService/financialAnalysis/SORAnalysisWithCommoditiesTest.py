"""
SOR (索提诺比率) 分析测试 - 支持股票 + 商品混合分析

本文件演示如何使用 SORAnalysis 进行多资产类别的投资组合索提诺比率分析
"""
...
class SORAnalysisWithCommoditiesTest:
    """SOR (索提诺比率) 分析测试类 - 支持商品数据"""

    def run_sor_with_commodities(self, stock_type="us_tech", start_date="20250101",
                                  end_date=None, risk_free_rate=None, interest_country=None,
                                  case_name=None, include_commodities=None):
        """
        执行 SOR (索提诺比率) 分析（支持股票 + 商品混合）
        ...
        logger.info("=" * 80)
        logger.info(" 开始 SOR (索提诺比率) 分析（支持商品）")
...
if __name__ == "__main__":
    """
    使用示例 - 批量生成带商品的 SR 分析报告
    """
    sorTest = SORAnalysisWithCommoditiesTest()
    ...
    for idx, config in enumerate(report_configs, 1):
        logger.info("\n" + "=" * 80)
        logger.info(f" 开始生成第 {idx}/{len(report_configs)} 个 SR 分析报告（含商品）")
        ...
        try:
            ...
            results = sorTest.run_sor_with_commodities(
