"""
SML 分析测试 - 支持股票 + 商品混合分析

本文件演示如何使用 SMLAnalysis 进行多资产类别的证券市场线分析
"""
...
class SMLAnalysisWithCommoditiesTest:
    """SML 分析测试类 - 支持商品数据"""

    def run_sml_with_commodities(self, stock_type="us_tech", start_date="20250101",
...
if __name__ == "__main__":
    """
    使用示例 - 批量生成带商品的 SML 分析报告
    """
    smlTest = SMLAnalysisWithCommoditiesTest()
    ...
    for idx, config in enumerate(report_configs, 1):
        logger.info("\n" + "=" * 80)
        logger.info(f" 开始生成第 {idx}/{len(report_configs)} 个 SML 分析报告（含商品）")
        ...
        try:
            ...
            results = smlTest.run_sml_with_commodities(
