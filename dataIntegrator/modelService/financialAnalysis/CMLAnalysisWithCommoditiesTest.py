"""
CML 分析测试 - 支持股票 + 商品混合分析

本文件演示如何使用 CMLAnalysis 进行多资产类别的投资组合分析
"""
...
class CMLAnalysisWithCommoditiesTest:
    """CML 分析测试类 - 支持商品数据"""

    def run_cml_with_commodities(self, stock_type="us_tech", start_date="20250101",
        ...
        results = cmlAnalysis.generate_cml_report(
            stocks=stocks,
...
if __name__ == "__main__":
    ...
    """
    cmlTest = CMLAnalysisWithCommoditiesTest()
    cmlAnalysisReport = CMLAnalysisReport()
    ...
    for idx, config in enumerate(report_configs, 1):
        ...
        try:
            ...
            results = cmlTest.run_cml_with_commodities(
            ...
            logger.info(f"   最大夏普比率: {results['max_sharpe_ratio']:.4f}")
    ...
    if all_results:
        ...
        comprehensive_pdf_path = cmlAnalysisReport.generate_comprehensive_pdf(
    ...
    logger.info(f"📁 报告输出目录: {cmlAnalysisReport.output_dir}")
    logger.info("=" * 80)
