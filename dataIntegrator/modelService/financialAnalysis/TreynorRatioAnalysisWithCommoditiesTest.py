"""
Treynor Ratio 分析测试 - 支持股票 + 商品混合分析

本文件演示如何使用 TreynorRatioAnalysis 进行多资产类别的特雷诺比率分析
"""
...
class TreynorRatioAnalysisWithCommoditiesTest:
    """特雷诺比率分析测试类 - 支持商品数据"""

    def run_treynor_with_commodities(self, asset_type="us_tech", start_date="20250101",
...
if __name__ == "__main__":
    """
    使用示例 - 批量生成带商品的特雷诺比率分析报告
    """
    treynorTest = TreynorRatioAnalysisWithCommoditiesTest()
    ...
    for idx, config in enumerate(report_configs, 1):
        logger.info("\n" + "=" * 80)
        logger.info(f"📊 开始生成第 {idx}/{len(report_configs)} 个特雷诺比率分析报告（含商品）")
        ...
        try:
            ...
            results = treynorTest.run_treynor_with_commodities(
    ...
    if all_results:
        logger.info("\n" + "=" * 80)
        logger.info(" 开始生成特雷诺比率综合分析研究报告（含商品资产）")
        logger.info("=" * 80)

        comprehensive_pdf_path = treynorAnalysisReport.generate_comprehensive_pdf(
            all_results=all_results,
            report_title="特雷诺比率 (Treynor Ratio) 综合分析研究报告（含商品资产）"
        )
