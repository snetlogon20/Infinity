...
class CMLAnalysisReport:
    ...
    def generate_cml_analysis_pdf(self, chart_paths, report_name, market_type="CN", start_date=None, end_date=None, output_path=None):
        ...
        - pdf_path: PDF 文件路径
        """
        self.job_logger.start_job('CMLAnalysisReport', 'CML',
    ...
    def generate_comprehensive_pdf(self, all_results, report_title="CML 综合分析研究报告", output_path=None):
        ...
        self.job_logger.start_job('CMLAnalysisReport_Comprehensive', 'CML',
                                  params={'cases_count': len(all_results),
                                          'start_date': first.get('start_date'),
                                          'end_date': first.get('end_date')})
        try:
            if not all_results:
                logger.warning("⚠️ 没有测试结果，无法生成综合报告")
                self.job_logger.end_job_failed("没有测试结果")
                return None
...
建议投资者结合自身风险承受能力，进行独立判断和决策。"""
            ...
            self.job_logger.end_job_success(records_processed=len(all_results))
            return output_path
        except Exception as e:
            import traceback
            self.job_logger.end_job_failed(str(e), traceback.format_exc())
